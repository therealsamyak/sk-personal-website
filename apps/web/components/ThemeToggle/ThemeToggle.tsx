...
export const ThemeToggle = () => {
  ...
  return (
    ...
    >
      {/* Switch knob */}
      <div className="flex size-7 items-center justify-center rounded-full bg-white shadow-lg transition-all duration-300 ease-in-out dark:translate-x-7 dark:bg-zinc-900">
        <Sun className="size-4 text-amber-500 transition-all duration-300 dark:scale-0 dark:opacity-0" />
        <Moon className="absolute size-4 scale-0 text-blue-300 opacity-0 transition-all duration-300 dark:scale-100 dark:opacity-100" />
      </div>

      {/* Background icons */}
      <Sun className="absolute left-2 size-3 text-white/70 opacity-0 transition-all duration-300 dark:opacity-60" />
      <Moon className="absolute right-2 size-3 text-black opacity-60 transition-all duration-300 dark:text-zinc-400 dark:opacity-0" />
